...
<body>
    <div class="noprint">
        <div class="row" style="float: right">
            <button onclick="window.print()" class="print-button"><span class="fas fa-print"
                    aria-hidden="true"></span>Print</button>
            <button>
                <a href="{{ route('admin.dashboard') }}" class=""><span class="fas fa-print"
    ...
    <div class="invoice-box">
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom: 20px;">
            ...
            <tr class="item last">
                <td colspan="2" style="border-right:none;">লাইসেন্স নং:{{ $license_details->license_no ?? 'N/A' }}
                    &nbsp|&nbsp লাইসেন্স ধরন: {{ $license_details->license_type }}
                </td>
                <td colspan="2" style="border-left:none;">জি/১ রশিদ নং:{{ $dd_j1_no ?? 'N/A' }},
            ...
            <tr class="item last">
                ...
                @if ($license_details->license_type == 'সংস্থা')
                    <td colspan="2" style="border-right:none;">সময়কাল:
                        {{ date('d-m-Y', strtotime($license_details['from_date'])) }} হতে
                        {{ date('d-m-Y', strtotime($license_details['to_date'])) }}
                    </td>
                @else
                ...
                <!-- <td colspan="2" style="border-right:none;">সময়কাল: ১লা
                    বৈশাখ {{ $license_date['fromDate'] }} হতে ৩০শে চৈত্র {{ $license_date['toDate'] }}</td> -->
                <td style="text-align:left;">লাইসেন্স পরিচিতি</td>
                <td>অর্থের পরিমাণ</td>
            </tr>
            ...
            <tr class="item last">
                ...
                <td style="border-bottom:none; text-align:left;">
                    ...
                    @if ($license_details->license_type == 'বাণিজ্যিক')
                        ...
                        মাস্টারপ্ল্যানের প্লট নং: {{ $license_details->license_mouja['com_license_plots'] ?? 'N/A' }}
                        <br>
                    @else
                ...
                <td style="width:200px; border-left:none;">
                    @if ($license_details->license_type == 'সংস্থা')
                        লাইসেন্স ফি : {{ $dd_no ?? 'N/A' }}<br>
                        ভ্যাট :{{ $dd_vat ?? 'N/A' }}<br>
                        উৎস কর : {{ $dd_tax ?? 'N/A' }}<br>
                    @else
                        লাইসেন্স ফি :{{ $dd_no ?? 'N/A' }} <br>
                        ভ্যাট : {{ $dd_vat ?? 'N/A' }}<br>
                        উৎস কর : {{ $dd_tax ?? 'N/A' }}<br>
                    @endif
                    ডিডি নং: {{ $dd_no ? $dd_no : 'N/A' }}<br>
            ...
            <tr class="item">
                <td colspan="2" style="border-right:none; font-weight: bold;">
                    @if ($license_details->user)
                        {{ $license_details->user->name }}, &nbsp {{ $license_details->user->designation }}, কর্তৃক
                        ডাটা
                        এন্ট্রি
                    @else
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom: 20px">
            ...
            <tr class="item last">
                ...
                <td style="text-align:left;">লাইসেন্স পরিচিতি</td>
                <td class="text-center">অর্থের পরিমাণ</td>
            </tr>
