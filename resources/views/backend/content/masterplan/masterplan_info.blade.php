...
<x-backend.card>
    ...
    <x-slot name="headerActions">
        <a href="{{ route('admin.masterplan.create') }}" class="btn btn-sm btn-primary">
            <i class="fa fa-plus" aria-hidden="true"></i> মাস্টারপ্লান যুক্ত করুন
        </a>
        <a href="{{ route('admin.masterplan.edit', $masterplan) }}" class="btn btn-sm btn-warning">
        ...
        <div class="container-fluid">
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <!-- <strong>জমির পরিমাণ: </strong>{{ number_format($masterPlanPlot->sum('total_sft')) ?? 'N/A' }} -->
                                </td>
                            </tr>
                            <tr>
                                <th>প্লট সংখ্যা</th>
                                <td>{{ count($masterplan->MasterPlanPlot) > 0 ? count($masterplan->MasterPlanPlot) : 'N/A' }}
                                </td>
                            </tr>

                            <tr>
                                <th>প্লটে মোট জমির পরিমাণ</th>
                                <td>{{ number_format($masterPlanPlot->sum('total_sft')) ?? 'N/A' }} বর্গফুট</td>
                            </tr>
                            <tr>
            ...
            <div class="row justify-content-end">
                <div class="col-md-2 mb-2">
                    <a href="{{ route('admin.masterplan.plot.create', $masterplan) }}" type="button" class="btn btn-primary float-right"><i class="fa fa-plus"></i>
                        Create New Plot</a>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <table class="table table-bordered" id="mouja-plot">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="form-control mr-1 plot_length ledger_amount" name="masterplan_plot[{{ $key }}][plot_length]" data-target="#total_sft_{{ $key }}" id="plot_length_{{ $key }}" value="{{ $plot->plot_length }}" placeholder="প্লটের দৈর্ঘ্য (ফুট)" data-height="#plot_width_{{ $key }}" data-target2="#plot_size_{{ $key }}">
                                </td>
                                <td>
                                    <input type="text" class="form-control ledger_amount plot_width" name="masterplan_plot[{{ $key }}][plot_width]" id="plot_width_{{ $key }}" value="{{ $plot->plot_width }}" placeholder="প্লটের প্রস্থ (ফুট)" data-target="#total_sft_{{ $key }}" data-target2="#plot_size_{{ $key }}" data-height="#plot_length_{{ $key }}">
                                </td>
                                <td>
                                    <input type="number" class="form-control" name="masterplan_plot[{{ $key }}][total_sft]" value="{{ $plot->total_sft }}" id="total_sft_{{ $key }}" placeholder="প্লটের পরিমাপ (বর্গফুট)" readonly>
                                    <input type="hidden" class="form-control" name="masterplan_plot[{{ $key }}][plot_size]" id="plot_size_{{ $key }}" value="{{ $plot->plot_size }}" placeholder="মোট জমির পরিমান">
                                </td>
