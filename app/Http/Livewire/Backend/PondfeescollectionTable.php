 ...
 * Class RolesTable.
 */
class PondfeescollectionTable extends DataTableComponent
{
  ...
  protected string $pageName = 'balam_pond';
  protected string $tableName = 'balam_pond';
  ...
  {
    return PondBalam::with('pondOwner');
  }
  ...
  public function columns(): array
  {
    ...
    return [
      ...
      Column::make(__('লাইসেন্স নং'), 'license_no')
        ...
        ->asHtml(),
      Column::make(__('লাইসেন্সের তথ্য'), 'pondOwner')
        ->addClass('text-left')
        ->searchable()
        ->format(function ($value, $column, $row) {
      ...
      Column::make('প্রক্রিয়া')
        ->addClass('text-left')
        ->format(function ($value, $column, $row) {
          return view('backend.content.pond.includes.feeactions')->with(['pond_license_fees' => $row]);
        }),
